...
public class ConfigPaymentMethodServiceImpl implements ConfigPaymentMethodService {
    ...
    public Mono<ConfigPaymentMethodDto> add(ConfigPaymentMethodDto configPaymentMethodDto) {
        return masterDataService.getMethodPayment(configPaymentMethodDto.getMethodPaymentId())
                .switchIfEmpty(Mono.error(
                        new ConfigMethodPaymentException("Method payment not found")
                ))
                .flatMap(
                        methodPaymentDto -> Mono.just(configPaymentMethodDto
                                )
    ...
    @Override
    public Mono<ConfigPaymentMethodDto> update(
            ConfigPaymentMethodDto configPaymentMethodDto, String id
    ) {
        ConfigPaymentMethod configPaymentMethod = mapperConfigPaymentMethod
                .toConfigPaymentMethod(configPaymentMethodDto);
        return masterDataService.getMethodPayment(
                    configPaymentMethodDto.getId()
                )
                .switchIfEmpty(Mono.error(
                    new ConfigMethodPaymentException("Method payment not found"))
                )
