...
public class TransactionServiceImpl implements TransactionService {
    ...
    @Override
    public Mono<Void> receiveTransactionConfirmation(
            TransactionBootcoinDto transactionBootcoinDto
    ) {
    ...
    public Mono<PayOrderDto> purchaseRequest(PayOrderDto payOrderDto) {
        return configPaymentMethodRepository.findByWalletId(payOrderDto.getSellerWalletId())
                .flatMap(configPaymentMethod ->
                        configPaymentMethodRepository
                                .findByWalletId(payOrderDto.getBuyerWalletId())
                        .flatMap(configPaymentMethod1 -> {
                            PayOrder payOrder = mapperPayOrder.toPayOrder(payOrderDto);
                            if (payOrderDto.getMethodPayment().equals(
                                    MethodPayment.TRANSFER.getAction()
                            )) {
                                payOrder.setConfigPayment(
                                        new ConfigPayment(
                                                configPaymentMethod1.getNumberAccount(),
                                                configPaymentMethod.getNumberAccount()
                                        )
                                );
                            } else if (
                                payOrderDto.getMethodPayment().equals(
                                        MethodPayment.YANKI.getAction()
                                )
                            ) {
                                payOrder.setConfigPayment(
                                        new ConfigPayment(
                                                configPaymentMethod1.getNumberCellPhone(),
                                                configPaymentMethod.getNumberCellPhone()
                                        )
                                );
                            }
